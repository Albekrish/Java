...
public class LoginView extends PasswordSecurity{
	...
	private void checkForLogin() {
		...
		case 1:
			...
			if (loginModel.adminValidation(userId, pw)) {
		...
		case 2:
			...
			if (loginModel.customerValidation(userId, pw)) {
		...
		case 3:
			...
			if (validateInput(userName, gender, dob, phNo, adhaarNo, mailId, panNo, street, city, state, pincode)) {
				userId = loginModel.newUserValidation(userName, gender, dob, phNo, adhaarNo, mailId, panNo, street,
						city, state, pincode);
				if (userId != 0) {
					String message = "";
		...
		if (custId == userId) {
			...
			if (pw.equals(message)) {
				...
				if (password.equals(rePassword)) {
					boolean flag = loginModel.addNewUser(userId, encrptPw);
					if (flag) {
