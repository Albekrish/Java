...
public class CourseLoginView implements CourseLoginViewCallBack {
	...
	public CourseLoginView() {
		loginController = new CourseLoginController(this);
	}
	...
	private void checkForLogin() {
		...
		case 1: // Admin
			...
			password = scan.next();
			if (loginController.adminValidation(userId, password)) {
		...
		case 2: // Student
			...
			System.out.println("Enter Password");
			password = scan.next();
			if (loginController.studentValidation(userId, password)) {
		...
		case 3: // Trainer
			...
			System.out.println("Enter Password");
			password = scan.next();
			if (loginController.trainerValidation(userId, password)) {
		...
		case 4:// new user sign-up page
			...
			if (password.equals(password1)) {
				if (loginController.addStudent(userId, password)) {
